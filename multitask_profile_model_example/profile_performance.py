...
# Modified by Katie - rather than printing metrics, this code returns them. No other changes!
...
def multinomial_log_probs(
    ...
    if return_cross_entropy:
        ...
        cross_ent = (-log_prob_pows_sum) / (trials+1)
...
def log_performance_metrics(metrics):
    ...
    # At this point, these metrics are all extracted from the dictionary and are (returned as list):
    final_metrics = []
    final_metrics.append(", ".join(
        [("%6.6f" % x) for x in nll]
    ))
    final_metrics.append(", ".join(
        [("%6.6f" % x) for x in ce]

...
if __name__ == "__main__":
    import argparse
    # SET VARIABLES
    parser = argparse.ArgumentParser(description='TF-MoDISco Hit Scoring')
    
    parser.add_argument('-s', '--shap-scores-path', type=str,
                        help='Path to saved shap scores')
    parser.add_argument('-t', '--tfm-results-path', type=str,
                        help='Path to saved TF-MoDISco results')
    parser.add_argument('-p', '--peak-bed-path', type=str,
                        help='Path to BED file of peaks')
    parser.add_argument('-o', '--outdir', type=str,
                        help='Path to output directory')
    parser.add_argument('-k', '--hyp-score-key', type=str,
                        help='Key under which hypothetical scores are stored, in the DeepSHAP scores file')
    parser.add_argument('-i', '--input-length', type=int, default=2114,
                        help='Length of input sequences for importance scores')
    parser.add_argument('-c', '--center-cut-size', type=int, default=400,
                        help='Length of sequence that was used to run TF-MoDISco')
    parser.add_argument('--keep-non-acgt', action='store_false',
                        help='SWITCH! If given, do not remove non-ACGT score tracks')
    parser.add_argument('-m', '--min-ic', type=int, default=0.2,
                        help='Information content cut-off to use to trim motif hits')
    parser.add_argument('-mc', '--metacluster-ind', type=int, default=0,
                        help='Index of the metacluster whose patterns to use for motif assignment; defaults to metacluster 0')
    parser.add_argument('-pt', '--pattern-inds', type=str, default=None,
    ...
    shap_scores_path = args.shap_scores_path
    tfm_results_path = args.tfm_results_path
    peak_bed_path = args.peak_bed_path
    outdir = args.outdir
    hyp_score_key = args.hyp_score_key
    input_length = args.input_length
    center_cut_size = args.center_cut_size
    keep_non_acgt = args.keep_non_acgt
    min_ic = args.min_ic
    metacluster_ind = args.metacluster_ind
    pattern_inds = args.pattern_inds

    print(f'Shap scores path: {shap_scores_path}\nTFM results path: {tfm_results_path}\n\
    Peak bed path: {peak_bed_path}\nOutput directory: {outdir}\nHyp score key: {hyp_score_key}\n\
    Input length: {input_length}\nCenter cut size: {center_cut_size}\nKeep non ACGT: {keep_non_acgt}\n\
    Min ic: {min_ic}\nMetacluster index: {metacluster_ind}\nPattern indexes: {pattern_inds}')

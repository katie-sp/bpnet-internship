...
os.chdir('/home/katie/bp_repo/multitask_profile_model_SPI_GATA/')
...
class BPDataset(torch.utils.data.Dataset):
    ...
    def _get_profiles_by_task(self, coords_batch, task):
        ...
        tf_neg_reader = pyBigWig.open(glob.glob(tasks_path + task + '/*neg.bw')[0], "r")
        tf_pos_reader = pyBigWig.open(glob.glob(tasks_path + task + '/*pos.bw')[0], "r")
    ...
    def _get_profiles(self, coords_batch, tasks):
        ...
        cont_profs_list = []
        for task in tasks:
            # check if it's a "_copy" task (for purposes of evaluation)
            if '_copy' in task:
                true_profs, cont_profs = self._get_profiles_by_task(coords_batch, task[:-5])
            else:
...
## PERFORMANCE EVALUATION FUNCTIONS - unchanged from Alex's code
...
def evaluate(train_tasks, val_tasks, test_tasks, num_tasks):
    ''' 
    ...
    combined_loss_and_metrics += LOSS_LIST
    ...
    metrics = profile_performance.compute_performance_metrics(
        ...
        statuses=statuses, model_task_statuses=test_tasks,
        print_updates=False
    )

    FINAL_METRICS = profile_performance.log_performance_metrics(metrics)
    combined_loss_and_metrics += FINAL_METRICS
    return combined_loss_and_metrics